...
z-idx は declarative な部分順序の z 関係を、拡張しても揺らがない数値スタック順位に変換する
直列チェーン、親 → 子ツリー、入れ子の TaggedPairs を受け取り、すべてのキー名を TypeScript 推論に持ち上げることで、
...
最初のトポロジカルパス（Kahn）がサイクルを拒否し、二度目のパスで各ノードの上下界を計算し、
...
二段の平衡木、子を伸ばした部分ファン、中間ノード分割、ジグザグとクロスリンクまで網羅。
各構成でトポロジ順の隣接間隔が等しく、辺が増えても根から葉までの単調性が崩れないことを確認。
